...
class G2PnetSolver:
    ...
    def optimize(self, max_iters = 600, tol=1e-3, stepsize_mode = "fixed", optimizor="gd_grass", verbose=False):
       '''
       ...
       if optimizor == "gd_grass":
           return self.optimize_gradient_descent_grass(max_iters, tol, stepsize_mode,  verbose)
       elif optimizor == "gd":
           return self.optimize_gradient_descent_Euclidean(max_iters, tol, stepsize_mode, verbose)  
       elif optimizor == "cg_grass":
           return self.optimize_conjugate_grad_grass(max_iters, tol, stepsize_mode, verbose)


 
    def optimize_gradient_descent_grass(self, max_iters=10, tol=1e-3, stepsize_mode = "fixed", verbose=False):
       '''
       ...
       self.hist_T_norm = np.zeros([max_iters])
       self.hist_cond_K = np.zeros([max_iters])
       self.hist_update = np.zeros([max_iters])

       messages = verbose
       if stepsize_mode == "fixed":
           eta = self.eta*np.ones([max_iters])
       elif stepsize_mode == "shrinkage":
           eta = np.array([self.eta/(np.log10(t+1)+1) for t in np.arange(max_iters) ])
       ...
       for i, eta_i in enumerate(eta):
           if i == 0:
              ...
              #eta_i = eta_i*self.hist_update[i-1]
           ...
           self.hist_T_norm[i] = self.T_norm
...
#              eta_i = eta_i*0.9
           ...
           if check_step(self.H, eta_i):
              ...
              U_temp, dU = Grassmann_update(self.U, self.H, eta_i)
              self.U = U_temp
              self.hist_update[i] = np.linalg.norm(dU)
    ...
    def optimize_conjugate_grad_grass(self, max_iters=10, tol=1e-3, stepsize_mode = "fixed", verbose=False):
       '''
       ...
       if stepsize_mode == "fixed":
           ...
           eta = np.array([self.eta/(2*np.log10(t+1)+1) for t in np.arange(max_iters) ])
       ...
       for i, eta_i in enumerate(eta):
           if i == 0:
              ...
              noise_var = self.Gaussian_noise_var
              self.hist_kern = np.vstack((self.hist_kern, self.kern.param_array))
              #eta_i = eta_i*self.hist_update[i-1]*10
              self.hist_eta[i] = eta_i
...
#              eta_i = eta_i*0.9
    ...
    def optimize_gradient_descent_Euclidean(self, max_iters=10, tol=1e-3, stepsize_mode = "fixed", verbose=False):
       '''
       ...
       for i, eta_i in enumerate(eta):
           if i == 0:
              ...
              noise_var = self.Gaussian_noise_var
              self.hist_kern = np.vstack((self.hist_kern, self.kern.param_array))
              #eta_i = eta_i*self.hist_T_norm[i-1]
              self.hist_eta[i] = eta_i
    ...
    def _gradient_U(self, mode=0):
       ...
       self.T_norm = np.linalg.norm(self.dL_dK) 
...
def Grassmann_update(U, H, eta):
    ...
    delta_U =  gutil.linalg.mdot(U_sig, Sig_sin, Vh_sig)
